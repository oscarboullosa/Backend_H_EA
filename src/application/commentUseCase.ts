...
export class CommentUseCase {
  ...
  public updateCommentPublication = async (
    uuid: string,
    {
      idUserComment,
    ...
    const commentValue = new CommentValue({
      uuid,
      idUserComment,
      idPublicationComment,
      textComment,
    ...
    const comment = await this.commentRepository.updateCommentPublication(
      uuid,
      commentValue
    );
    if (!comment) {
  ...
  public insertCommentPublication = async ({
    uuid,
    idUserComment,
    idPublicationComment,
    textComment,
    ...
    const comment = await this.commentRepository.insertCommentPublication(
      commentValue
    );
    if (!comment) {
      throw new NotFoundError("Comment not found");
  ...
  public responseComment = async (
    ...
    const comment = await this.commentRepository.responseComment(
      uuid,
      commentValue
    );
    if (!comment) {
  ...
  public listResponsesPag = async (uuid: string, numPage: string) => {
    const listComment = await this.commentRepository.listResponsesPag(
      uuid,
      numPage
    );
    return listComment;
  ...
  public getCommentPublicationByIdPag = async (
    uuidPublication: string,
    numPage: string
  ) => {
    const comment = await this.commentRepository.getCommentPublicationByIdPag(
      uuidPublication,
      numPage
    );
    return comment;
