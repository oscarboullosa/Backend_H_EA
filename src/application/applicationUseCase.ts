...
export class ApplicationUseCase {
  ...
  public insertApplication = async ({
    uuid,
    idSender,
    idReceiver,
    typeApplication,
    ...
    const applicationValue = new ApplicationValue({
      uuid,
      idSender,
      idReceiver,
      typeApplication,
    ...
    const activity = await this.applicationRepository.insertApplication(
      applicationValue
    );
    if (!activity) {
  ...
  public deleteApplication = async (uuid: string) => {
    const application = await this.applicationRepository.deleteApplication(
      uuid
    );
    return application;
  };
  ...
  public listApplicationPag = async (numPage: string) => {
    const listApplication = await this.applicationRepository.listApplicationPag(
      numPage
    );
    return listApplication;
  };
  ...
  public getNumApplications = async () => {
    const numApplication =
      await this.applicationRepository.getNumApplications();
