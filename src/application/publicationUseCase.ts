...
export class PublicationUseCase {
  ...
  public getPublicationById = async (uuid: string) => {
    const publication = await this.publicationRepository.getPublicationById(
      uuid
    );
    if (!publication) {
  ...
  public updatePublication = async (
    ...
    const publicationValue = new PublicationValue({
      uuid,
      idUser,
      likesPublication,
      textPublication,
  ...
  public insertPublication = async ({
    uuid,
    idUser,
    likesPublication,
    textPublication,
    ...
    const publicationValue: PublicationValue = new PublicationValue({
      uuid,
      idUser,
      likesPublication,
      textPublication,
  ...
  public deletePublication = async (uuid: string) => {
    const publication = await this.publicationRepository.deletePublication(
      uuid
    );
    return publication;
  };
  ...
  public getNumPublications = async () => {
    const numPublications =
      await this.publicationRepository.getNumPublications();
  ...
  public listPublicationsPag = async (numPage: string) => {
    const listPublication =
      await this.publicationRepository.listPublicationsPag(numPage);
    return listPublication;
  };

  public updateLikes = async (uuid: string, uuidUser: string) => {
    const publication = await this.publicationRepository.updateLikes(
      uuid,
      uuidUser
    );
    return publication;
  ...
  public deleteLikes = async (uuid: string, uuidUser: string) => {
    const publication = await this.publicationRepository.deleteLikes(
      uuid,
      uuidUser
    );
    return publication;
