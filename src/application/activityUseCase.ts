...
export class ActivityUseCase {
  ...
  public getActivitiesByLocation = async (idLocation: string) => {
    ...
    if (!activities) {
      ...
      const activitiesByLocation = activities.filter(
        (activity) => activity.idLocation?.toString() === idLocation.toString()
      );
  ...
  public updateActivity = async (
    ...
    const activity = await this.activityRepository.updateActivity(
      uuid,
      activityValue
    );
    if (!activity) {
  ...
  public getParticipantsOfActivity = async (uuid: string, numPage: string) => {
    const participants =
      await this.activityRepository.getParticipantsOfActivity(uuid, numPage);
    return participants;
  };

  public getActivitiesByUserAndWeek = async (uuid: string, startDate: Date) => {
    const activities = await this.activityRepository.getActivitiesByUserAndWeek(
      uuid,
      startDate
    );
    return activities;
  ...
  public getFollowedUsersActivities = async (
    uuid: string,
    page: string,
    startDate: Date
  ) => {
    const activities = await this.activityRepository.getFollowedUsersActivities(
      uuid,
      page,
      startDate
    );
