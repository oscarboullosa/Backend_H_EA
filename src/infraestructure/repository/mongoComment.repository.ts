...
export class MongoCommentRepository implements CommentRepository {
  async getCommentPublicationByIdPag(
    ...
    const responseItem = await CommentModel.find({
      idPublicationComment: uuidPublication,
    })
      .skip(hop)
  ...
  async updateCommentPublication(
    ...
    const responseItem = await CommentModel.updateOne({ _id: uuid }, data, {
      new: true,
    });
    return responseItem;
  }
  ...
  async updateLikes(uuid: string, uuidUser: string): Promise<any> {
    const responseItem = await CommentModel.findOneAndUpdate(
      { _id: uuid },
      { $addToSet: { likesComment: uuidUser } },
      { new: true }
    );
    return responseItem;
  }
