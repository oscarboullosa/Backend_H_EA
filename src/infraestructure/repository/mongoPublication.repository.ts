...
export class MongoPublicationRepository implements PublicationRepository {
  async getPublicationById(uuid: string): Promise<any> {
    const response = await PublicationModel.findOne({ _id: uuid }).populate(
      "idUser"
    );
    return response;
  }
  ...
  async updatePublication(uuid: string, data: PublicationEntity): Promise<any> {
    const response = await PublicationModel.findOneAndUpdate(
      { _id: uuid },
      data,
      { new: true }
    );
  ...
  async insertPublication(data: PublicationEntity): Promise<any> {
    ...
    const response = await PublicationModel.updateOne(
      { _id: item._id },
      updatedData
    );
  ...
  async getOwnPosts(uuid: string): Promise<any> {
    ...
    const publications = await PublicationModel.find({ idUser: uuid })
      .populate("idUser")
      .exec();
    return publications;
  }
